...
{
    //Bu servis icerisinde subscribe islemini yurutecegiz
    public class EventStoreBackgorundService(IEventStoreService eventStoreService ,IMongoDBService mongoDBService) : BackgroundService
    {
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            await eventStoreService.SubscripeToStreamAsync("products-stream", async (streamSubsciprion, resolvedEvent, cancellationToken) =>
            {
                ...
                switch (@event)
                {
                    ...
                    case AvailablilityChangedEvent e:
                        product = await (await productCollection.FindAsync(p => p.Id == e.ProductId)).FirstOrDefaultAsync();
